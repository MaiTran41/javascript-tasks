...
const showPlanetIndex = (planet) => {
  console.log(`Planet: ${planet}, Index: ${planets.indexOf(planet)}`);
};

planets.map(showPlanetIndex);
...
mixedValues.forEach((value) => {
  console.log(`Item: ${value}, Index: ${mixedValues.indexOf(value)}
    , Type: ${typeof value}`);
});
...
*/
// Write your code here
const animalNameLetter = (name) => {
  const formattedLetters = name.split("").join(", ");
  console.log(`Letters in '${name}': ${formattedLetters}`);
};

animalNames.forEach(animalNameLetter);
...
const getPositiveTemperatures = (temperatureArr) => {
  console.log(temperatureArr.filter((temp) => temp > 0));
};
...
const indexOfFox = zooAnimals.indexOf("Fox");
...
const findAnimalIndex = (animalArr, animal) => {
  if (animalArr.indexOf(animal) === -1) {
    console.log(`${animal} is not in the zoo`);
  }
};

findAnimalIndex(zooAnimals, "Penguin");
...
*/
// Write your code here
const sortedAnimalArr = safariAnimals.sort();
...
const reversedAnimalArr = sortedAnimalArr.reverse();
console.log(reversedAnimalArr);
